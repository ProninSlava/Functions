...
def read_write(documents, directories):
    info_list = documents
    info_list1 = directories
    dt_now = datetime.datetime.now()
    with open('info.json', 'a', encoding='UTF-8') as f:
        json.dump(str(dt_now), f, ensure_ascii=False, indent=3)
        json.dump(info_list, f, ensure_ascii=False, indent=3)
        json.dump(info_list1, f, ensure_ascii=False, indent=3)

    print('Вы записали информацию в файл!')
...
def com(documents, directories):
    ...
    while n < 20:
        ...
        print('\t\t\tВЫБЕРИТЕ КОМАНДУ: \ndl-list     выведет список всех документов в определенном формате'
              ...
              '\nw–write     записать данных в файл'             
              '\nq-exit      выход')
        print('______________________________________________________________________________________')
        print(f'Запрос №{n}')
        ...
        if commanda == 'p':
            ...
            read_write(documents, directories)
